...
import com.hiacc.embeddings.umap.metric.Metric;
...
import com.hiacc.embeddings.umap.metric.CategoricalMetric;
import com.hiacc.embeddings.umap.metric.EuclideanMetric;
import com.hiacc.embeddings.umap.metric.Metric;
import com.hiacc.embeddings.umap.metric.PrecomputedMetric;
import com.hiacc.embeddings.umap.metric.ReducedEuclideanMetric;
...
//  private final Float mB = null;
    ...
    public Matrix getmGraph() {
        return mGraph;
    }

    public Matrix getmRawData() {
        return mRawData;
    }

    public void setmRawData(Matrix mRawData) {
        this.mRawData = mRawData;
    }

    public void setmGraph(Matrix mGraph) {
        this.mGraph = mGraph;
    }
    ...
    static IndexedDistances nearestNeighbors(final Matrix instances, final int nNeighbors, final Metric metric, boolean angular, final Random random, final int threads, final boolean verbose) {
        if (verbose) {
            UmapUtils.message("Finding nearest neighbors");
        }
        ...
        if (metric.equals(PrecomputedMetric.SINGLETON)) {
            ...
            knnIndices = UmapUtils.fastKnnIndices(instances, nNeighbors);
...
//        knnDists = (float[][]) nn[1];
                ...
                if (verbose) {
                    UmapUtils.message("Building random projection forest with " + nTrees + " trees");
                }
                ...
                if (verbose) {
                    ...
                    UmapUtils.message("Total number of values in forest: " + nodeCount);
                    UmapUtils.message("NN descent for " + nIters + " iterations");
                }
            ...
            if (MathUtils.containsNegative(knnIndices)) {
                UmapUtils.message("Failed to correctly find nearest neighbors for some samples. Results may be less than ideal. Try re-running with different parameters.");
            }
        }
        if (verbose) {
            UmapUtils.message("Finished nearest neighbor search");
        }
...
//    }
    ...
    private void fit(Matrix instances, float[] y) {
        ...
        if (mVerbose) {
            UmapUtils.message("Starting fitting for " + instances.rows() + " instances with " + instances.cols() + " attributes");
        }
...
//    }
        ...
        if (instances.rows() <= mNNeighbors) {
            ...
            UmapUtils.message("nNeighbors is larger than the dataset size; truncating to X.length - 1");
        ...
        if (mVerbose) {
            UmapUtils.message("Construct fuzzy simplicial set: " + instances.rows());
        }
        UmapProgress.update();
        ...
        if (instances.rows() < SMALL_PROBLEM_THRESHOLD) {
            ...
            if (mMetric == PrecomputedMetric.SINGLETON) {
                UmapUtils.message("Using precomputed metric; transform will be unavailable for new data");
            } else {
        ...
        if (mVerbose) {
            UmapUtils.message("Construct embedding");
        }
        ...
        if (mVerbose) {
            UmapUtils.message("Finished embedding");
        }
    ...
    public Matrix transformLoadW(Matrix instances) {
        ...
        if (mSmallData) {
            ...
            dists = UmapUtils.submatrix(distanceMatrix, indices, mRunNNeighbors);
        } else {
