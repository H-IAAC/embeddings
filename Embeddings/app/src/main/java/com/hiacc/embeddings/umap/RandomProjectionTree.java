...
final class RandomProjectionTree {
    ...
    private static Object[] angularRandomProjectionSplit(final Matrix data, final int[] indices, final Random random) {
        ...
        float leftNorm = UmapUtils.norm(data.row(left));
        float rightNorm = UmapUtils.norm(data.row(right));
        ...
        float hyperplaneNorm = UmapUtils.norm(hyperplaneVector);
    ...
    static List<FlatTree> makeForest(final Matrix data, final int nNeighbors, final int nTrees, final Random random, final boolean angular) {
        final Random[] randoms = UmapUtils.splitRandom(random, nTrees);  // insure same set of random numbers for 1 and multiple threads
        ...
        try {
            ...
            UmapUtils.message("Random Projection forest initialisation failed due to recursion limit being reached. Something is a little strange with your data, and this may take longer than normal to compute.");
